...
def train():
	...
	root = opt.root
	#data_dir = root + 'gingivitus/'
	#data_dir="D:/PGGAN-PyTorch/gingivitus/dataset/"
	#data_dir="D:/PGGAN-PyTorch/Gingivitis/dataset/"
	data_dir="D:/FAST/PGGAN-PyTorch/finalDS/dataset/"
	print(data_dir)
...
## The schedule contains [num of epoches for starting each size][batch size for each size][num of epoches]
	schedule = [[5, 15, 25 ,35, 40],[2, 2, 2, 2, 2],[5, 5, 5, 1, 1]]
	...
	device = torch.device('cuda' if (torch.cuda.is_available() or opt.cuda)  else 'cpu')
	print(device)
	...
	try:
		...
		data_loader = DataLoader(dataset=dataset, batch_size=batch_size, shuffle=True, num_workers=workers)
		print("TRY:",dataset)
